...
const categoryMeta: Record<string, { title: string; subtitle: string; tone: string }> = {
  hair: { title: "Hair Services", subtitle: "Cuts, color, wash & care", tone: "from-emerald-500/20" },
  beard: { title: "Beard Services", subtitle: "Trim, sculpt, style", tone: "from-emerald-500/15" },
          ...
          {categoryOrder.map((cat) => {
            ...
            return (
              <SectionReveal key={cat}>
                <div className={`rounded-3xl border border-white/5 bg-gradient-to-br ${meta.tone} to-transparent p-6 sm:p-8`}>
                  <div className="mb-6 flex items-end justify-between">
                    <div>
                      <h3 className="heading-md">{meta.title}</h3>
                      <p className="text-sm text-white/55">{meta.subtitle}</p>
                    </div>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    {items.map((s) => (
                      ...
                      >
                        ...
                        <p className="mt-4 text-xs text-white/45">{s.duration}</p>
                      </Link>
                    ))}
                  </div>
                </div>
